...
public class StaffController {
	...
	public ModelAndView reviewPatientRecord(HttpServletRequest request) {
		...
		List<Doctor> doctors = doctorDAO
				.searchDoctorsByStaffPersonId(person_id);
		model.addObject("doctors", doctors);
		model.addObject("content", "review_patient");
		return model;
	}

	@RequestMapping(value = "/assign_patient", method = { RequestMethod.GET,
			RequestMethod.POST })
	...
	@RequestMapping(value = "/select_patient/{doctorId}", method = {
			RequestMethod.GET, RequestMethod.POST })
	public ModelAndView selectPatient(HttpServletRequest request,
	...
	// @RequestMapping(value = "/edit_profile_action", method =
	// RequestMethod.POST)
	...
	@RequestMapping(value = "/create_appointment_form_1", method = {
			RequestMethod.GET, RequestMethod.POST })
	...
	@RequestMapping(value = "/see_appointment", method = { RequestMethod.GET,
			RequestMethod.POST })
	public ModelAndView seeAppointment(
		...
		if (!keyword.equals("")) {
			List<Visit> appointments = appointmentDAO
					.getAppoinmentsByPatientName(keyword);
	...
	@RequestMapping(value = "/appointment_schedule/{appointmentId}", method = {
			RequestMethod.GET, RequestMethod.POST })
	public ModelAndView seeAppointmentbyId(@PathVariable int appointmentId,
			HttpServletRequest request) {
		HttpSession session = request.getSession();
		Person user = (Person) session.getAttribute("user");
		if (user.getRoleID() != 3)
		...
		ModelAndView model = new ModelAndView("staff/index");
		// if (!keyword.equals("")) {
		// List<Visit> appointments =
		// appointmentDAO.getAppoinmentsByPatientName(keyword);
		// model.addObject("appointments", appointments);
		// }
		Visit appointment = appointmentDAO.getAppointment(appointmentId);
		List<Map<Integer, Object>> schedule = basicService.findScheduleByVisit(
				1, appointment.getDoctorId());
		Patient patient = patientDAO.getPatientsByPatientId(appointment
				.getPatientId());
		model.addObject("schedule", schedule);
		model.addObject("appointment", appointment);
		model.addObject("patient", patient);
		model.addObject("dayDiff",
				FormatService.findDiffDays(new Date(), appointment.getDate()));
		model.addObject("content", "see_appointment");
		return model;
	}

	@RequestMapping(value = "/reschedule/{patientId}", method = {
			RequestMethod.GET, RequestMethod.POST })
	...
	@RequestMapping(value = "/appointment_delete/{appointmentId}", method = {
			RequestMethod.GET, RequestMethod.POST })
	...
	public ModelAndView createAppointmentTwo(@PathVariable int personId,
			HttpServletRequest request) {
		...
		Doctor doctor = doctorDAO.getDoctorById(patient.getDefaultDoc());
		// List<Visit> visits = appointmentDAO.getAppointmentsByDoctorId(docId,
		// todayString, lastDayString);
		List<Visit> appointments = appointmentDAO
				.getAppoinmentsByPatientId(patientID);
		// List<Long> dateDiffList = new ArrayList<Long>();
		// for (Visit appointment : appointments) {
		// dateDiffList.add(FormatService.findDiffDays(new Date(),
		// appointment.getDate()));
		// }
		List<Map<Integer, Object>> schedule = basicService.findScheduleByVisit(
				1, doctor.getId());
	...
	public String appointmentFormSubmit(
			HttpServletRequest request,
			@RequestParam(value = "time", defaultValue = "") String time,
			@RequestParam(value = "type", defaultValue = "") String type,
			@RequestParam(value = "appointmentId", defaultValue = "") String appointmentId,
			@ModelAttribute("visit") Visit visit, BindingResult result)
